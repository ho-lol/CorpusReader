...
def make_dict(result_dic,raw_array,tagged_array):
    ...
    for raw_sent,tagged_sent in zip(raw_array,tagged_array):
        ...
        for raw_word,tag_word in zip(raw_sent,tagged_sent):
            ...
            elif not include_delete(opcodes):
                        ...
                        if prev != None:
                            ...
                            if nxt!=None: #replace,insert 이런경우도 잇을까?
                                if nxt[0]=="insert":
                                    ...
                                    print(merge_morph)
                                    print(opcodes)
                            if  prev_dic.count('++'): #+가 없을때 요건 좀 애매한데 이런경우 잇을까?
                                print(raw_word)
                                print(merge_morph)
                                print(opcodes)
...
##                            print(opcodes)
                ...
                for morph_tag in tag_morph:##일단 잘라주고
                    morph,tag = str2tuple(morph_tag)
                    for syl in morph:
                        fraction.append([syl,tag])
                    fraction[-1][0] = fraction[-1][0] + '+'
                ...
                if len(block_list) != 0:
                    ...
                    while(len_word != len(mat_blocks)):
                        ...
                        for start,end,block_temp in block_list:
                            if(len_word == start):
                                blocks.pop()
...
##                    fraction = []
##                    ##맨마지막 매칭 블록 삭제
##                    mat_block = SM.get_matching_blocks()
##                    len_block = len(mat_block)-1
##                    len_pyochung = len_block * 2 - 1
##                    if mat_block[-1][2]>=1:
##                        print(mat_block)
##                    pyochung_list = []
##                    dic_list = []
##                    tag_list = []
##                    for cur,nxt in izip(mat_block,islice(mat_block,1,None)):
##                        
##                        
##
##                    
...
##                        if len(i)!=2:
##                            
##                    #j1가 merge_morph에서 시퀸스가 달라지는 지점 j2가 시퀸스가 같아지는 지점
##
##                    ##앞처리를 먼저하자.
##
##                    ##짜개지는부분 처리
